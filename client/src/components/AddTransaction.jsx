...
const AddTransaction = ({ userId, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name || !amount) {
      alert("Please fill in both title and amount fields.");
      return;
    }
  ...
  return (
    <div className="mt-10 max-w-3xl mx-auto p-8 rounded-3xl shadow-2xl bg-gradient-to-br from-white to-slate-100 dark:from-slate-800 dark:to-slate-900 border border-slate-200 dark:border-slate-700 transition-all duration-500 relative">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Tags */}
        <div>
          <label className="block mb-3 text-sm font-semibold text-slate-600 dark:text-slate-300">
            Tags
          </label>
        ...
        <div>
          <button
            type="submit"
            className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold text-lg shadow-md hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
