...
import api, { getSummary, getBudgetGoals } from "../api/api";
...
const CategoryBudgetGoals = ({ goals, setGoals }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      console.error(
        "Failed to update category goals:",
  ...
  const totalBudget = Object.values(goals).reduce(
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {defaultCategories.map((category) => (
          <div key={category} className="flex flex-col gap-2">
            <label className="text-base font-medium text-slate-700 dark:text-white">
              {category}
            </label>
            <input
              type="number"
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const goalsRes = await getBudgetGoals();

        console.log("Summary:", summaryRes);
        console.log("Goals:", goalsRes);

        const { totalBudget, totalIncome, totalExpenses } = summaryRes;
        setBudget(totalBudget);
        setTotalIncome(totalIncome);
        setTotalExpenses(totalExpenses);

        const formattedGoals = (goalsRes.categoryGoals || []).reduce(
          (acc, curr) => {
      ...
      <div className="bg-gradient-to-b from-slate-50 to-white dark:from-[#0c0f1c] dark:to-[#1a1d2e] border border-slate-200 dark:border-slate-700 p-10 rounded-3xl shadow-xl flex flex-col md:flex-row gap-10">
        ...
        <div className="w-full md:w-2/3">
          ...
          <div className="relative w-full bg-slate-200 dark:bg-slate-700 h-4 rounded-full overflow-hidden mb-8 shadow-sm">
            <div
              className={`h-full ${
                remainingBudget < 0 ? "bg-red-500" : "bg-indigo-500"
          ...
          <div className="mt-6 bg-gradient-to-tr from-white/60 to-slate-200/60 dark:from-slate-800/60 dark:to-slate-700/60 backdrop-blur-md p-6 rounded-xl text-center shadow-sm">
            ...
            <p className="text-sm font-medium text-slate-700 dark:text-slate-300">
              ...
              of your budget for consistent savings!
            </p>
          </div>
        </div>
